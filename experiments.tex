...
Since the videos contain a non-equal number of frames, we fixed the number of frames inside the batch for each video (video clip length) taken into account, in order to be able to train the model with a batch size larger than one.
...
The batch size is $[B, V, H, W]$, where they are batch size, video clip length, height and width, respectively.
Unless otherwise specified, the following configuration is fixed: input video is $320 \times 240$, video clip length is 8, LSTM cell number is 2, $\mathit{NF}$ is 3, linear weights are initialized using a uniform distribution, the LSTM cells with a (semi-)orthogonal matrix, bias parameters are set to zero and the VST is initialized with a model pretrained on Something-Something v2~\cite{goyal2017something}.
...
We verified that, in conjunction with the use of NF as 4 (best configuration found in the previous experiment), it permits to obtain higher AUC than with 2 cells.
We speculate this happen because both (3 cells and NF 4 models) reach the best AUC at same time (around 400 epochs).
...
In fact, the highest AUC is obtained using 8 frames as video clip length, which represents a good trade-off between enlarging clip size and exploiting LSTM cells while, at the same time, avoiding overfitting due to consecutive frames being too similar to each other.\mnote{poi me la spiegate}
