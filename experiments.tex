...
\centering
	...
	\caption{Performance comparison changing the number and type of (long-term) memory cells (from 1 to 4). If not defined means no memory cell (neither following Linear) is inside the model. \lnote{FIX: Row \#4} is $NF 3$ of previous experiment in Figure \ref{fig:num-frames-vst}.}
...
\lnote{se con gru non cambia molto, nel grafico metterei solo lstm e descriverei nel testo il risultato, perchè altrimenti è troppo confusionario}
