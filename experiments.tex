...
We perform our tests on DoTA dataset \cite{9712446}, using only the anomaly class and its temporal boundaries.
...
We train our model using a weighted Cross-Entropy loss to address the issue of imbalanced data within the DoTA dataset, assigning $w_n=0.3$ and $w_a=0.7$ to the normal and anomaly class respectively.
...
We had tested in advace the effect of both memories.
As shown in Table \ref{tab:short-vs-long-term-auc}, STMM and LTMM both contribute to enhance the general performance, obtaining the best AUC when both are active, highlighting the need for both. 
...
As expected, taking into account only the current frame is the worst situation, losing any temporal information and making the training unstable.
...
As mentioned in \ref{Long-Term Memory Module descr}, according to our hypotheses, overloading the transformer becomes harmful, as it does not have a mechanism to weight the past tense differently.
...
We verified this hypothesis and, in conjunction with the use of $\mathit{NF}=4$ (best configuration found in the previous experiment), it permits to obtain higher AUC than with 2 cells.
...
The training of MOVAD followed the best configuration found using information obtained through previous ablation studies: VCL of 8, 3 LSTM cells and $\mathit{NF}=4$.
...
It is worth to note that, unlike previous SOTA models, our model also process videos in which traffic participants are not present or visible without any problem.
