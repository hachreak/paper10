...
Of all the annotations available, we use only the anomaly class and its temporal boundaries.
...
Our model training and evaluation take place in the online scenario.
...
We train our model using a weighted Cross-Entropy loss to address the issue of imbalanced data within the DoTA dataset. Specifically, we assign $w_n=0.3$ to the normal class and $w_a=0.7$ to the anomaly class, to balance the training.
...
\end{tabular}
    \end{adjustbox}
    \hspace{0.5em}
	\caption{Detection accuracy (AUC) for individual accident categories. ``*'' indicates non-ego anomaly categories. ``\dag'' when input resolution is $640\times480$ instead of $320\times240$. N/A=Not Available. Bold and red values are the best and second-best results.}
...
As shown in Table \ref{tab:short-vs-long-term-auc}, STMM and LTMM both contribute to enhance the general performance. 
...
\tnote{In Fig.~\ref{fig:num-frames-vst}, results when varying the number of frames $\mathit{NF}$ processed by the VST at each step are displayed.}
As expected, taking into account only the current frame is the worst situation, making the training unstable.
\tnote{This is reasonable: by not having any knowledge of the near past, the STMM leads the LTMM to overfitting since consecutive frames are very similar.}
...
On the contrary, increasing the number of frames generally increase the performance. This is true until processing 5 frames, where the effect becomes counterproductive.
...
\tnote{In Fig.~\ref{fig:num-memory-cells} the LTMM on the classification capability are evaluated, varying the number of cells from zero (no LSTM at all) to four.}
...
\tnote{In Fig.~\ref{fig:random-batch}, different values of VCL are explored.}
...
\tnote{The training of MOVAD followed the best configuration found using information obtained through previous ablation studies: VCL of 8, 3 LSTM cells and $\mathit{NF}=4$.}
...
Both MOVAD surpass the SOTA models.
...
The classes are grouped by ego and non-ego anomaly categories.
It is interesting to note that our model, unlike previous SOTA models, is not limited to videos in which anomalous traffic participants are present and visible.
For this reason, the table shows the results obtained by MOVAD for the classes VO and OO.
