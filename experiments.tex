...
Our tests are related to the Task~1~\cite{9712446}, the (frame-level) Video Anomaly Detection.
...
\begin{figure}[b!]
\centering
	\includegraphics[trim=0 0 0 0, clip, width=0.7\columnwidth]{images/exp_1.jpg}
...
\begin{figure}[b!]
\centering
	\includegraphics[trim=0 0 0 0, clip, width=0.7\columnwidth]{images/exp_2.jpg}
    % TODO aggiorna caption!
	\caption{Performance comparison, changing the number of LSTM cells (from 0 to 4) that form the long-term memory. "LSTM 2 cells" corresponds to NF 3 in Fig.~\ref{fig:num-frames-vst}.\mnote{forse per simmetria la taglierei anche questa sui 400 come le altre due.}\anote{Concordo}}
	\label{fig:num-memory-cells}
\end{figure}

\noindent\textbf{Training details.}
Since the videos contain a non-uniform number of frames, we fixed the number of frames for each video taken into account (video clip length) in order to be able to train the model with a batch size larger than one.
	...
	\begin{center}
		\begin{tabular}{|!l||^c|^c|^c|^c|^c|^c|^c|^c|^c|^c|^c|^c|^c|^c|^c|^c|^c|^c|}
			...
			\hline\hline
                ...
                STFE \cite{zhou_spatio-temporal_2022} & 75.2 & \red{84.5} & 72.1 & 77.3 & 72.8 & 71.9 & N/A & N/A & N/A & \textbf{80.6} & 65.6 & \red{69.9} & 76.5 & 74.2 & N/A & \red{75.6} & \red{70.5} & N/A \\
                % v22_2 epoca 370
                ...
                % v30_2 (epoca 450)
                \textbf{Our (MOVAD) **}                  &  \textbf{86.6} & \textbf{86.3} & \textbf{84.9} & \textbf{83.7} & \textbf{85.5} & \red{81.6} & \red{77.4} & \textbf{87.9} & \red{73.8} & 72.2 & \textbf{74.0} & \textbf{74.8} & \textbf{80.2} & \textbf{79.6} & \textbf{76.8} & \textbf{82.2} & \textbf{78.3} & \textbf{72.9} \\
                % v30_2 (epoca 450)
...
\end{tabular}
	\end{center}
	\caption{Detection accuracy (AUC) for individual accident categories. "*" indicates non-ego anomaly categories. "**" indicates input resolution is $640\times480$ instead of $320\times240$. N/A=Not Available. Bold and red values are the best and second-best results.\lnote{aggiungi 30\_3 al posto della 20\_2}}
...
By increasing the number of cells, the maximum peak of AUC is reached slower, but its absolute value is higher. The maximum AUC is reached, even if with just +$0.02\%$ compared to 3 LSTM cells, by 2 LSTM cells around epoch 270.
...
The worst and most unstable training is obtained with 4 frames, probably because they are too few to exploit the long-term memory effect of LSTM cells. \anote{Ma non ho capito ... prendiamo solo 4 frames per ogni video??? O 16? Ma non sono troppo pochi per vedere un'anomalia? Se è così io valuterei se togliere tutto questo esperimento, anche per motivi di spazio}
...
Both are trained with the following winning configuration of: video clip length of 8, 3 LSTM cells and $NF=4$.
