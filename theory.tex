...
The network has to take the time into consideration, as incorrect behavior is often recognizable by taking into consideration the immediate past. \mnote{sposterei da "since the task..." a qui prima di dire che abbiamo scelto i trasformer per fare questo}
...
\mnote{forse sposterei qui la sezione head (o meglio la parte prima del pezzo finale) adattandola visto che il flusso di elaborazione è quello per poi appunto introdurre i sottomoduli lstm e saliency} 

\noindent\textbf{Long-term memory.}
In order to not lose information from the distant past, we needed a way to keep track of it.
Because the model is working in an online fashion, this time the requirement is opposite compared to the short-term memory.\mnote{non capisco bene cosa si intenda, forse userei la frase dopo come seconda parte di questa, ma in generale  rigirerei un po' questa parte iniziale}
...
\tnote{Quindi questa parte si può togliere?}
...
Taking inspiration from DRIVE~\cite{bao2021drive}, we added more information about the scene present in the current frame with the help of a saliency model, used in evaluation mode \anote{cioe'?}.
The fixation information tells us which regions of the frame attract a person's attention, simulating what human see at first glance.\mnote{introdurre fixation, forse spostare sopra}
...
As seen in the Figure \ref{fig:arch-saliency}, we normalized and concatenated the output of MLNet last layer and the fourth last layer, to form a single saliency vector $s[t]$ of 2275 values\anote{Io spiegherei questo numero, magari con un footnote}.
Before the concatenation, the features are passed through a max pooling and average pooling \anote{Cioè li usiamo entrambi? Perchè? Non li inseriamo in Fig. 1?}, respectively.
...
For each frame at time $t$, the final output is the anomaly classification score $s_t \in [0,1]$ \anote{Dobbiamo usare un altro simbolo perchè s lo abbiamo già usato prima per la saliency}, where $s_t=0$ means absence of anomaly and $s_t=1$ means the frame is surely anomalous.
