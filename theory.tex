...
The first memory module is able to keep track of both the present and the near past, while the second one will be able to also handle the remote past.\anote{Evidenzierei poi nella nuova versione della Fig. 1 i 4 blocchi di cui si parla qui} Indeed, an overview of the overall architecture is shown in Figure~\ref{fig:arch}.
...
More in detail, only a small portion of recent frames are fed as input to the short-term memory module.\mnote{coordinare ??? con figura}
...
Specifically, our choice to design this module fell on an RNN module (LSTM in this specific case \mnote{GRU? CITAZIONE?}).
