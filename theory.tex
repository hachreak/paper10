...
In order to implement this module, we selected  the VST~\cite{liu_video_2022} over ViViT~\cite{Arnab_2021_ICCV} \vnote{as our backbone network} due to its superior performance \vnote{and more efficient computation of self-attention mechanism.
We also favored VST over an RNN due to its ability to process frames in parallel.}
...
\vnote{Despite its efficient computation, the self-attention mechanism remains computationally intensive, particularly for long videos. Therefore, we have opted to utilize VST to encode features from small video sequences into a compact representation that will subsequently be forwarded to the LTMM.}
...
The model internally splits the frames in non-overlapping 3D patches, partitioning the video in $\frac{\mathit{NF}}{2} \times \frac{H}{4} \times \frac{W}{4}$ 3D tokens, projecting the features to an arbitrary dimension $C$\vnote{, where $C$ denotes the channel number of the hidden layers in the first stage and varies depending on the version of VST used.}.
...
\vnote{We aim to encourage a finer classification by considering the contextual information from previous events in the driving scene. To formulate this context, we repeatedly transfer the compact representation obtained from the STMM to our LTMM.
Over time, the latter will accumulate a richer understanding of the scene, enabling more precise classifications.}
