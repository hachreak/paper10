...
\begin{figure}[!ht]
        ...
        \caption{The online video frame anomaly detection architecture. $f[t]$ is the frame at time $t$, $x$ the output of the Reducer, $T$ the number of frames in input to the VST, $s[t]$ the anomaly classification score of the frame $f[t]$. \anote{da modificare in figura da $T$ a $NF$ in basso} \mnote{L'ho messa io singola colonna per vedere quanto spazio risparmiavamo. EDIT: anche modificata e croppata spostando l'I/O dell'LSTM (ha senso stressare RNN?) di modo da stringerla e aggiunto VST. }}
		\label{fig:arch}
\end{figure}
