...
In this paper, in order to build a system that is capable of recognizing different types of anomalous situations \lnote{non so se dire "capable of recognizing different types of anomalous situations", perchè alla fine le usiamo tutte come se fossero appartenenti a una classe "anomalia", non addestriamo la rete a riconoscere la classe} and locate them temporally in the video, we have introduced two main architectural contributions. The first exploits a Video Swin Transformer (VST) network \cite{liu_video_2022}, adapted to model short-term memory. This adaptation allows the system to incorporate temporal correlation and to reproduce online behaviors, by relying only on the current and few previous frames.
