...
\begin{itemize}
    \item the use of an additional intermediate representation of the current frame scene given by saliency model \cite{cornia2016deep} to enrich the information available to the final classifier;
    \item the adapting of the Video Swin Transformer model \cite{liu_video_2022} to efficiently encode short-term memory, represented by current and few previous frames;
    \item the use of a LSTM module, injected inside the classifier head, to model the long-term memory of the past;
    \item an exhaustive ablation study on all the components of the architecture;
    \item a novel architecture to work in a real-time and online scenario for online video anomaly detection is proposed.
\end{itemize}

The paper is organized as follows.
Section \ref{sec:related}, we will first introduce all the previous related work.
Section \ref{sec:theory}, it will follow an overview of the proposed architecture and an in-depth explanation of all the components employed.
